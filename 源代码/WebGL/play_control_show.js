    ...
    return new Promise((resolve, reject) => {
        fetch(url)
            ...
            .then(blob => {
                ...
                const bandwidth = fileSize / downloadTime * 0.75;
                resolve(bandwidth);
            })
            .catch(reject);
    });
...
export async function fetchAndParseMPD_show(mpdUrl) {
    try {
...
export async function play_control_tbb(representations) { //TBB-ABR algorithm
    startFPSCounter();
    ...
    for (let chunkStart = 1450; chunkStart <= 1749; chunkStart += 5) {
